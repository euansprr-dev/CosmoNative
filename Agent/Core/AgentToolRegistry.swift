...
class AgentToolRegistry {
    ...
    private var workspaceEditingTools: [LLMToolDefinition] {
        [
            LLMToolDefinition(
                ...
                description: "Stage reviewed edits for the active Cosmo surface (document edits, number/fact replacements, hook changes, outline changes, filling in sections, FORMATTING). Put ALL changes for the request in ONE call — pass multiple items in the operations array rather than calling this tool several times. EDIT IN PLACE, ONE OPERATION PER CHANGED LINE: for each change, set `originalText` to the ENTIRE line/sentence that contains the target, copied VERBATIM from the current surface text, and `proposedText` to that SAME line with ONLY the requested change applied — preserve every other word of the user's copy unless they explicitly ask to rewrite/rework wording. NEVER fuse several lines into one big replacement when only one of them changes (the app splits such blocks back into per-line edits and rejects the leftovers), NEVER reorder or move existing lines unless the user explicitly asked (a deliberate move requires `explicitMove: true` on the operation AND a removal covering the line's current position), and NEVER re-add a line that already exists near the edit — that duplicates it and the proposal is rejected. Anchoring on the whole line is what lets the edit locate and land in place; a bare fragment (like just a number) often fails to match and gets dumped at the bottom flagged as outdated. Use `textInsertion` only to add brand-new content, with `originalText` set to the existing text the new content should follow. FORMATTING (bold/italic/underline/strikethrough/headings): use kind `formatMarks` with `formatMark` set and `originalText` = the exact text to format (whole line for line-level formatting like headers) — the words stay identical, only their appearance changes. NEVER express formatting by rewriting text with asterisks or markdown symbols. To format EVERY slide header at once (e.g. \"bold all the headers\"), send ONE `formatMarks` operation with `scope` set to `allSlideHeaders` — the app expands it to every header line exactly. When inserting outline points under existing slide headers, anchor to the existing `SLIDE N` line but do NOT include another `SLIDE N` in proposedText; insert only the body copy for that slide. RENUMBERING a sequential series (inserting or removing a slide / numbered step): give any brand-new item its correct integer number, then add ONE `renumberSequence` operation — `seriesKind` `slideHeaders` or `numberedSteps`, `fromNumber` = the first number that must shift, `delta` = 1 to shift up (or -1 to shift down), optional `withinSlide` for step lists inside a slide — and the app rewrites every affected line from the live document. NEVER hand-copy individual header bumps, never invent a fractional number (no `SLIDE 6.5`, no `Step 5b`), and never leave two items sharing a number. The result includes `resultingDocumentStructure` — check it matches the user's ask and stage a corrected proposal if not. Never regenerate the whole document as one giant operation, and never send a `textReplacement` without `originalText`. This tool never applies changes; it only creates a user-reviewed proposal.",
                parametersSchema: [
                    "type": "object",
                    "properties": [
                        ...
                        "operations": [
                            ...
                            "items": [
                                "type": "object",
                                "properties": [
                                    "kind": ["type": "string", "enum": ["textReplacement", "textInsertion", "structuredFieldReplacement", "formatMarks", "renumberSequence", "canvasPlan"], "description": "textReplacement: replace originalText with proposedText in place. textInsertion: insert proposedText immediately after the originalText anchor. structuredFieldReplacement: replace a structured field. formatMarks: apply formatting (formatMark) to originalText without changing the words — proposedText is not needed. renumberSequence: shift every number in a sequential series (slide headers or numbered steps) by delta, expanded by the app from the live document — use this instead of hand-writing per-item bumps. Prefer textReplacement for wording changes, formatMarks for appearance changes."] as [String: Any],
                                    "targetID": ["type": "string"] as [String: Any],
                                    "anchorID": ["type": "string"] as [String: Any],
                                    "originalText": ["type": "string", "description": "For textReplacement: the EXACT existing text to replace, copied verbatim (with punctuation) from the surface text. For textInsertion: the EXACT existing text to insert immediately after (the anchor). For formatMarks: the EXACT existing text to format. Required whenever you touch existing content — omit only to append brand-new content at the very end."] as [String: Any],
                                    "proposedText": ["type": "string", "description": "The new text this operation produces. Omit for formatMarks — the words don't change."] as [String: Any],
                                    "formatMark": ["type": "string", "enum": ["bold", "italic", "underline", "strikethrough", "heading1", "heading2", "heading3"], "description": "For formatMarks operations: the formatting to apply to originalText."] as [String: Any],
                                    "explicitMove": ["type": "boolean", "description": "Set true ONLY when the user's request itself asks to move/reorder content and this operation re-adds a line that another operation removes from its current position. Without it, any operation that relocates an existing line is rejected."] as [String: Any],
                                    "scope": ["type": "string", "enum": ["allSlideHeaders"], "description": "For formatMarks only: apply the mark to every SLIDE header line at once — one operation formats them all."] as [String: Any],
                                    "seriesKind": ["type": "string", "enum": ["slideHeaders", "numberedSteps"], "description": "For renumberSequence: which sequential series to shift."] as [String: Any],
                                    "fromNumber": ["type": "integer", "description": "For renumberSequence: the first number in the series that must shift."] as [String: Any],
                                    "delta": ["type": "integer", "description": "For renumberSequence: 1 shifts numbers up (item inserted), -1 shifts them down (item removed)."] as [String: Any],
                                    "throughNumber": ["type": "integer", "description": "For renumberSequence: optional last number to shift — omit to shift through the end of the series."] as [String: Any],
                                    "withinSlide": ["type": "integer", "description": "For renumberSequence with seriesKind numberedSteps: the slide whose step list is being renumbered."] as [String: Any],
            ...
            LLMToolDefinition(
                name: "pull_evidence",
                description: "Pull the material the user has ALREADY gathered about the concept being developed: staged seedbed captures, matching extracts from the connection's deep dive, AND matching highlights from their Readwise bookshelf (kind \"readwise\" — always name the book and author when citing these: 'you highlighted almost exactly this in Make It Stick'). Call this while developing a concept to probe with the user's own material, or when they ask what they have on the topic. Rows arrive only when the concept's own words appear in them — an empty bookshelf result means nothing relevant exists; never invent a quote. Evidence NEVER enters the page silently: cite it in conversation, and only stage it (via propose_workspace_edit, quoting it under Evidence with its source) after the user says yes.",
                parametersSchema: [
                    "type": "object",
                    "properties": [
    ...
    func toolsForIntent(_ intent: AgentIntent, source: MessageSource = .inApp) -> [LLMToolDefinition] {
        ...
        case .brainstorm:
            tools = coreTools + ideaTools + swipeTools + captureTools + clientTools + clientProfileTools + intelligenceTools + writingTools + clientMemoryTools + preferenceTools + scoringTools + lessonTools + moduleManagementTools + webSearchTools
        case .plan:
            tools = scheduleTools + contentTools + strategyTools + clientProfileTools + lessonTools + moduleManagementTools + workspaceTools
        case .query:
            return allTools
        case .correct:
            tools = ideaTools + contentTools + contentDataTools + scheduleTools + clientTools + clientProfileTools + moduleManagementTools + workspaceTools
        case .execute:
            tools = contentTools + contentDataTools + scheduleTools + writingTools + clientProfileTools + uxTools + lessonTools + moduleManagementTools + workspaceTools + workflowTools
        case .debrief, .reflect:
            tools = coreTools + analyticsTools + lessonTools + moduleManagementTools
        case .meta:
            tools = preferenceTools + standingInstructionTools + clientMemoryTools + lessonTools + uxTools + moduleManagementTools
        case .strategy:
            tools = coreTools + strategyTools + swipeTools + contentTools + analyticsTools + intelligenceTools + writingTools + clientProfileTools + clientMemoryTools + insightMemoryTools + uxTools + lessonTools + moduleManagementTools + webSearchTools
        case .draft:
        ...
        case .research:
            tools = coreTools + swipeTools + ideaTools + captureTools + clientProfileTools + insightMemoryTools + webSearchTools + lessonTools
        case .synthesize:
            tools = coreTools + swipeTools + ideaTools + intelligenceTools + insightMemoryTools + clientProfileTools + lessonTools + webSearchTools
        case .analyze:
            tools = coreTools + intelligenceTools + swipeTools + analyticsTools + contentTools + clientProfileTools + insightMemoryTools + lessonTools + moduleManagementTools + webSearchTools + sqlTools
        case .organize:
            tools = workspaceTools + contentTools + ideaTools + scheduleTools + uxTools + workflowTools + automationTools
        }
