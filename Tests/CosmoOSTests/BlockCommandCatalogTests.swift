...
final class BlockCommandCatalogTests: XCTestCase {
    ...
    func testSlashCommandMappingRoutesBlockTransformationsThroughBlockEditor() {
        XCTAssertEqual(BlockCommandCatalog.action(for: .heading1), .transform(.heading1))
        XCTAssertEqual(BlockCommandCatalog.action(for: .heading2), .transform(.heading2))
        XCTAssertEqual(BlockCommandCatalog.action(for: .heading3), .transform(.heading3))
