...
final class ConnectionStagingGateTests: XCTestCase {
    ...
    // MARK: - Synthesis section (V3)

    func testSynthesisSectionSerializesAndAcceptsStagedBullets() {
        let text = model.text
        let synthesisHeader = ConnectionSurfaceSerializer.headerLine(for: .synthesis)
        XCTAssertTrue(text.contains(synthesisHeader), "the board must expose Synthesis to the collaborator")
        // Conclusion placement: after Open Questions, before the meta sections.
        let openQ = text.range(of: ConnectionSurfaceSerializer.headerLine(for: .openQuestions))!
        let synth = text.range(of: synthesisHeader)!
        let name = text.range(of: ConnectionSurfaceSerializer.headerLine(for: .conceptName))!
        XCTAssertTrue(openQ.lowerBound < synth.lowerBound && synth.lowerBound < name.lowerBound)

        let op = operation(
            kind: .textInsertion,
            anchorID: ConnectionSurfaceSerializer.anchorID(for: .synthesis),
            proposedText: "- Acting from the desired reality closes the gap the excuses defend"
        )
        XCTAssertTrue(issues([op]).isEmpty, "approved realizations stage into Synthesis like any section")
    }
