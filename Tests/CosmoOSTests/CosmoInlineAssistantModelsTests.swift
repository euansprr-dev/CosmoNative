...
final class CosmoInlineAssistantModelsTests: XCTestCase {
    ...
    func testDiffEngineBuildsParagraphReplacementHunks() {
        ...
        // LCS diff groups removals before additions when no lines are shared.
        XCTAssertEqual(hunks.map(\.kind), [.removed, .removed, .added, .added])
        XCTAssertEqual(hunks[0].text, "Rent: $4,556/mo")
        XCTAssertEqual(hunks[1].text, "Expenses: $1,800/mo")
        XCTAssertEqual(hunks[2].text, "Rent: $5,000/mo")
