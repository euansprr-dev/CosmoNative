...
/// Display-only chrome floated over the text (placeholder, ghost tail) must
/// never be the click target: under SwiftUI's window, a click that hit-tests
/// to a label reaches the text view only as a responder-chain forward, and
/// that path never makes the field first responder — the window-initiated
/// pre-focus on a direct hit does. Click-through keeps every click on the
/// direct-hit path (Today quick-add "can't click in" fix, July 2026).
private final class WashPassthroughLabel: NSTextField {
...
final class WashTextView: NSTextView {
    ...
    let placeholderLabel: NSTextField = WashPassthroughLabel(labelWithString: "")
    private let ghostLabel: NSTextField = WashPassthroughLabel(labelWithString: "")
