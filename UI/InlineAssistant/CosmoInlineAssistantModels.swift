        ...
        case .concept:
            return CosmoInlineAssistantSkill(
                ...
                instructions: [
                    ...
                    "WHERE A POINT GOES: run this ladder IN ORDER and stop at the first match. A specific instance, story, scenario, or person doing the thing → Examples. A step, a sequence, or 'do X then Y' → Process. Support that cites a source, a study, or the user's own observed experience → Evidence. A pain, cost, or failure the concept addresses → Problems. A positive outcome it produces → Benefits. A doubt, counterargument, or 'people will say...' → Beliefs & Objections. Something unresolved the user is still wondering about → Open Questions. Only when NONE of those fit AND the concept would collapse if the statement were false does the point go to Claims: a claim is a load-bearing assertion the framework rests on, never a default bin for declarative sentences. If a candidate claim mostly restates or elaborates an existing Claims bullet, stage it as a textReplacement revision of that bullet (or put it where the ladder pointed), not as a new claim. PRESSURE CHECK: when Claims already holds more entries than any other section on the surface, re-run the ladder once before staging anything else into Claims; a Claims section twice the size of its neighbors means routing has drifted, and you should say so if the user asks why the board feels lopsided. Synthesis, Concept Name, and References are never ladder targets: Concept Name only through the naming driver, References only as link rows, Synthesis only through the distill moves below.",
                    ...
                    "SYNTHESIS IS THE ANSWER TO THE GOAL, EARNED SLOWLY. The Synthesis section holds the concept's conclusions: 3 to 7 realizations that answer the Goal line, each a single sentence the user could hand someone as the payoff of the whole board. It is never a routine capture target and you never stage into it silently. Exactly two moves put material there. (1) RESONANCE: when the user lands a line that answers the Goal (they say a reframe kills the whole excuse, or they agree when you name a line as the tightest on the board), offer it in your pane reply: 'that feels like one of the concept's core realizations, want it in Synthesis?' Stage into Synthesis only after their yes, phrased from their own words. (2) THE DISTILL RITUAL: when the user asks to distill, conclude, or sum up the concept (or accepts your offer to), first propose 3-5 realizations in the pane, each drawn from accepted board entries, naming what each one distills ('this pulls together the placebo claim and the painter example'); let them react, then stage the approved ones as bullets into Synthesis in one operation. While distilling, when a realization makes existing Claims bullets redundant, offer the fold: name which claims it absorbs and stage those removals or revisions only on their yes, never silently. KEEP IT SCARCE: if Synthesis already holds 7 entries, a new realization replaces or merges an existing one (say which, and why) instead of growing the list. When the board is mature (Goal filled, several sections carrying material, objections handled) and Synthesis is still empty, offer the distill once; if they decline, do not offer again until the board has meaningfully grown.",
