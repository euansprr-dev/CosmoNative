...
const ALL_TOOLS: ToolDefinition[] = [
  ...
  { name: 'create_content', description: 'Create new content atom. Pass blueprintTitles with the swipe/blueprint names the user mentions — UUIDs are resolved internally.', parameters: { type: 'object', properties: { title: { type: 'string' }, body: { type: 'string' }, platform: { type: 'string' }, clientName: { type: 'string' }, sourceIdeaUUID: { type: 'string' }, description: { type: 'string' }, framework: { type: 'string' }, hooks: { type: 'array', items: { type: 'string' } }, swipeUUIDs: { type: 'array', items: { type: 'string' } }, blueprintTitles: { type: 'array', items: { type: 'string' }, description: 'Blueprint swipe titles — engine resolves to UUIDs internally' } }, required: ['title'] } },
  ...
  { name: 'update_content', description: 'Update content atom', parameters: { type: 'object', properties: { uuid: { type: 'string' }, title: { type: 'string' }, body: { type: 'string' }, platform: { type: 'string' }, clientName: { type: 'string' }, description: { type: 'string' }, framework: { type: 'string' }, hooks: { type: 'array', items: { type: 'string' } }, outline: { type: 'array', items: { type: 'string' } }, researchBriefing: { type: 'string', description: 'Research data or context for the writing engine' } }, required: ['uuid'] } },
  ...
  // === WRITING ===
  { name: 'generate_outline', description: 'Generate a content outline. Pass blueprint swipe TITLES via blueprintTitles — the engine resolves UUIDs internally. No need to search_swipes first.', parameters: { type: 'object', properties: { contentUUID: { type: 'string' }, notes: { type: 'string', description: 'User structural instructions (e.g. "2-3 slides year by year", "step-by-step process")' }, clientName: { type: 'string' }, blueprintTitles: { type: 'array', items: { type: 'string' }, description: 'Blueprint swipe TITLES — engine resolves to UUIDs internally. Preferred over blueprintSwipeUUIDs.' }, blueprintSwipeUUID: { type: 'string', description: 'Single primary blueprint UUID (use blueprintTitles instead)' }, blueprintSwipeUUIDs: { type: 'array', items: { type: 'string' }, description: 'Array of blueprint UUIDs (use blueprintTitles instead)' }, contentFormat: { type: 'string', enum: ['reel', 'carousel', 'thread', 'post'] } }, required: ['contentUUID'] } },
  { name: 'generate_draft', description: 'Write a full draft', parameters: { type: 'object', properties: { contentUUID: { type: 'string' }, clientName: { type: 'string' }, userDirection: { type: 'string' }, contentFormat: { type: 'string' }, researchBriefing: { type: 'string', description: 'Research data, factual context, or briefing for the writing engine to use as source material' } }, required: ['contentUUID'] } },
